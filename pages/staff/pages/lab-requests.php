...
// Fetch lab requests
$stmt = $conn->prepare("SELECT lr.*, p.first_name AS patient_first, p.last_name AS patient_last, d.first_name AS doctor_first, d.last_name AS doctor_last FROM lab_requests lr JOIN patients p ON lr.patient_id = p.id JOIN doctors d ON lr.doctor_id = d.id WHERE lr.status IN ('requested', 'in_progress') ORDER BY lr.requested_at DESC");
$stmt->execute();
